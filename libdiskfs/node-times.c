...
/* If the disk is not readonly and noatime is not set, then check relatime
   ...
   If the file is a directory and nodiratime is set, return false.
   */
int
atime_should_update (struct node *np)
{
  ...
  if (_diskfs_nodiratime && S_ISDIR (np->dn_stat.st_mode))
    return 0;
...
/* If disk is not readonly and the noatime option is not enabled, set
   ...
   more recent than atime. If nodiratime is enabled and NP is a directory,
   ignore NP->dn_set_atime. */
void
