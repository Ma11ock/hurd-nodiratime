...
struct parse_hook
{
  int readonly, sync, sync_interval, remount, nosuid, noexec, noatime,
    noinheritdirgroup, relatime, nodiratime;
};
...
set_opts (struct parse_hook *h)
{
  ...
  if (h->nodiratime != -1)
    _diskfs_nodiratime = h->nodiratime;
...
parse_opt (int opt, char *arg, struct argp_state *state)
{
  ...
  switch (opt)
      ...
      else
	{
	  ...
	  h->sync_interval = -1;
	  h->remount = 0;
	  h->nosuid = h->noexec = h->noatime = h->noinheritdirgroup = h->relatime = h->nodiratime = -1;
